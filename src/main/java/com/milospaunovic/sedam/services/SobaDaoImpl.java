...
public class SobaDaoImpl implements SobaDao{
    ...
    public List<Soba> loadActiveFromTo(int from) {
        int page = (from - 1)*20;
        List<Soba> lista = session.createCriteria(Soba.class).setFirstResult(page).setMaxResults(20).addOrder(Order.asc("id")).setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
        return lista;
